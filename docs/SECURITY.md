...
- 依存関係は `requirements.lock` と `pnpm-lock.yaml` をrelease sourceとする。push／PRのrelease gateでpip-audit／pnpm audit／gitleaksを確認し、同じ依存監査を毎日と手動実行でも先行確認する。Dependabotが毎日作成するpatch／minor更新PRを放置せず、CI成功後に取り込む。
