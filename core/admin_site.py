...
class BackofficeAdminSite(AdminSite):
    ...
    def get_urls(self):
        ...
        custom = [
            path(
                ...
                self.admin_view(self.direct_image_presign),
                name="direct_image_presign",
            ),
            path(
                "image-uploads/complete/",
                self.admin_view(self.direct_image_complete),
                name="direct_image_complete",
            ),
            path(
                "image-uploads/delete/",
                self.admin_view(self.direct_image_delete),
                name="direct_image_delete",
            ),
        ]
        return custom + urls
    ...
    def _image_api(self, callback):
        ...
        try:
            ...
            logging.getLogger("restfull.image_uploads").exception(
                "unexpected_image_api_error"
            )
            response = JsonResponse(
    ...
    def direct_image_delete(self, request):
        from core.direct_image_uploads import delete_upload
    ...
    def index(self, request, extra_context=None):
        ...
        if can_view_orders:
            ...
            context.update(
                ...
                .distinct()
                .count(),
                customer_waiting=orders.filter(
                    status=Order.Status.AWAITING_PAYMENT
                ).count(),
                shipped_waiting=orders.filter(status=Order.Status.SHIPPED).count(),
                recent_orders=orders.exclude(
                    status__in=(
                        Order.Status.COMPLETED,
                        Order.Status.CANCELLED,
                        Order.Status.REFUNDED,
                    )
                ).order_by("dashboard_priority", "created_at")[:8],
            )

        if can_view_payments:
            context["sales_30_days"] = (
        ...
        if can_view_inquiries:
            ...
            context.update(
                ...
                recent_inquiries=inquiries.exclude(
                    status__in=(Inquiry.Status.COMPLETED, Inquiry.Status.SPAM)
                )[:5],
            )
        ...
        if can_view_products:
            ...
            context.update(
                low_stock_count=low_stock.count(), low_stock_products=low_stock[:5]
            )
