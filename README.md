...
`/admin/`はDjangoのデータ・権限・操作ロジックを維持した「靜院居家 運営管理」です。ダッシュボードに対応待ち注文、新着問い合わせ、30日間の入金、在庫アラート、LINE通知失敗を集約しています。各ユーザーに表示される数値と導線は従来と同じDjangoモデル権限に従います。

注文詳細では現在の注文状態に応じた次の操作が表示されます。通知送信やキャンセルの前に確認ダイアログが表示されますが、在庫・金額・LINE通知の処理は従来と同じサーバー側操作を使用します。
